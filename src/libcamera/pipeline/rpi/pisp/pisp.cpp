...
const std::unordered_map<unsigned int, BayerFormat> BayerToMbusCodeMap{
	...
	{ MEDIA_BUS_FMT_PISP_COMP1_BGGR, { BayerFormat::BGGR, 16, BayerFormat::Packing::PISP1 } },
	{ MEDIA_BUS_FMT_PISP_COMP1_GBRG, { BayerFormat::GBRG, 16, BayerFormat::Packing::PISP1 } },
	{ MEDIA_BUS_FMT_PISP_COMP1_GRBG, { BayerFormat::GRBG, 16, BayerFormat::Packing::PISP1 } },
	{ MEDIA_BUS_FMT_PISP_COMP1_RGGB, { BayerFormat::RGGB, 16, BayerFormat::Packing::PISP1 } },
};
...
unsigned int bayerToMbusCode(const BayerFormat &bayer)
{
	const auto it = std::find_if(BayerToMbusCodeMap.begin(), BayerToMbusCodeMap.end(),
				     [bayer](const std::pair<unsigned int, BayerFormat> &match) {
						return bayer == match.second;
				     });
...
PiSPCameraData::platformValidate(RPi::RPiCameraConfiguration *rpiConfig) const
{
	...
	if (!rawStreams.empty()) {
		 ...
		 */
		BayerFormat sensorBayer = BayerFormat::fromMbusCode(rpiConfig->sensorFormat_.mbus_code);
		if (flipsAlterBayerOrder_) {
			sensorBayer.order = nativeBayerOrder_;
			sensorBayer = sensorBayer.transform(rpiConfig->combinedTransform_);
		}
...
int PiSPCameraData::platformPipelineConfigure(const std::unique_ptr<YamlObject> &root)
{
	...
	if (config_.minTotalCfeBuffers < config_.minCfeBuffers) {
		LOG(RPI, Error)
			<< "Invalid configuration: min_total_cfe_buffers must be >= min_cfe_buffers";
		return -EINVAL;
	}

	if (config_.minTotalCfeBuffers < 1) {
		LOG(RPI, Error)
			<< "Invalid configuration: min_total_cfe_buffers must be >= 1";
		return -EINVAL;
	}
