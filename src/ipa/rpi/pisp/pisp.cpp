...
{
	std::scoped_lock<RPiController::Metadata, BackEnd> l(rpiMetadata, *be_);
...
RPiController::StatisticsPtr IpaPiSP::platformProcessStats(Span<uint8_t> mem)
{
	...
	for (i = 0; i < statistics->focusRegions.numRegions(); i++)
		statistics->focusRegions.set(i, { stats->cdaf.foms[i], 0, 0 });
...
{
	if (controls.contains(controls::AE_METERING_MODE)) {
...
{
	std::scoped_lock<BackEnd, FrontEnd> l(*be_, *fe_);
...
void IpaPiSP::setStatsAndDebin()
{
	...
	for (unsigned int i = 0; i < sizeof(agc.weights); i++)
		agc.weights[i] = 0x11; /* Weights for 2 regions per 8-bits */
